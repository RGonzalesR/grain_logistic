...
def criar_feature_store(spark: SparkSession):
    ...
    df_feature = (
        ...
        .join(
            df_full, 
            (df_full["ds_corr"] == df_corredor_data["ds_corredor_de_armazenagem"]) 
            & (df_full["dt_completa"] <= df_corredor_data["dt_historico"]),
            "inner"
        )
        .groupBy("ds_corredor_de_armazenagem", "dt_historico")
        .agg(
            f.count(f.col("dt_completa")).alias("qt_envios_acumulado"),
            f.sum(f.col("qt_produtos_adquiridos")).alias("qt_total_itens"),
            f.avg(f.col("qt_produtos_adquiridos")).alias("qt_media_itens"),
            f.sum(f.col("vl_receita_total")).alias("vl_total_receita"),
            f.avg(f.col("vl_receita_total")).alias("vl_media_receita"),
            f.avg(f.col("vl_peso_total")).alias("vl_peso_medio"),
            f.avg(f.col("qt_tempo_entrega_dias")).alias("vl_medio_tempo_entrega"),
            f.avg(f.col("in_chegou_no_tempo").cast(IntegerType())).alias("pc_entregas_no_prazo"),
            f.avg(f.col("qt_ligacoes_do_cliente")).alias("vl_medio_ligacoes_cliente"),
            f.avg(f.col("nr_avaliacao_do_cliente")).alias("vl_medio_avaliacao_cliente"),
            f.avg(f.col("nr_avaliacao_entrega")).alias("vl_medio_avaliacao_entrega"),
            f.sum(f.when((f.col("ds_genero") == "F"), 1).otherwise(0)).alias("qt_envios_feminino"),
            f.sum(f.when((f.col("ds_genero") == "M"), 1).otherwise(0)).alias("qt_envios_masculino")
        )
